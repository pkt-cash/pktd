...
func OpenFile(path string, readOnly bool) (Storage, error) {
	...
	if !readOnly {
		logw, err = os.OpenFile(filepath.Join(path, "LOG"), os.O_WRONLY|os.O_CREATE, 0600)
		if err != nil {
			return nil, err
		}
...
func (fs *fileStorage) doLog(t time.Time, str string) {
	...
	if fs.logw == nil {
		var err error
		fs.logw, err = os.OpenFile(filepath.Join(fs.path, "LOG"), os.O_WRONLY|os.O_CREATE, 0600)
		if err != nil {
			return
		}
...
func (fs *fileStorage) setMeta(fd FileDesc) error {
	...
	if _, err := os.Stat(currentPath); err == nil {
		...
		if err := writeFileSynced(currentPath+".bak", b, 0600); err != nil {
			fs.log(fmt.Sprintf("backup CURRENT: %v", err))
			return err
		}
	...
	if err := writeFileSynced(path, []byte(content), 0600); err != nil {
...
func (fs *fileStorage) Create(fd FileDesc) (Writer, error) {
	...
	of, err := os.OpenFile(filepath.Join(fs.path, fsGenName(fd)), os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		return nil, err
	}
	fs.open++
