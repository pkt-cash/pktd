...
build() { l="${1:-}"; printf '%s\n' "Building ${l:?${unset:?}}"; o=$(printf '%s\n' "${l:?${unset:?}}"|sed 's/^pktd$/./'); go build -a ${RACE} -o "${bindir:?${unset:?}}"/"${l?${unset:?}}" -trimpath -ldflags="${PKTD_LDFLAGS:?${unset:?}} -w -s -buildid=" "./${o?${unset:?}}" || die "Failed building ${l?${unset:?}}"; }
...
export CGO_ENABLED=0 && export RACE=""
...
go test ${RACE} ${PKTD_TESTFLAGS:?${unset:?}} ./... || die "One or more pktd tests failed."
if [ -n "${RUN_GOLEVELDB_TESTS:-}" ]; then # shellcheck disable=SC2086,SC2248
    { { cd goleveldb; go test ${RACE} ${PKTD_TESTFLAGS:?${unset:?}} ./... || die "One or more GoLevelDB tests failed"; } && cd ..; }
fi
