...
if [ -z "${TEST_LDB}" ]; then LDBTEST='goleveldb' && export LDBTEST; else
	LDBTEST="_X_X_X_X_" && export LDBTEST; fi
...
cleanUp || true; GOTAR=$( $STDBUF go list ./... | $STDBUF grep -v test | $STDBUF grep -v $LDBTEST | $STDBUF grep -v $LGTTEST | grep -v $LNDTEST | $STDBUF sort | $STDBUF uniq) 
# shellcheck disable=SC2090,SC2086
( $STDBUF gocov test -v -vet=off $TESTF $GOTAR 1>gocov_report_pktd.json ) 2>&1 | $STDBUF tr '\t' ' ' | $STDBUF tr -s ' ' | $STDBUF sed 's/github.com\/pkt-cash\/pktd\///' | $STDBUF sed 's/github.com\/pkt-cash\///' | $STDBUF grep -v '\[no test files\]' #| $STDBUF sed 's/ coverage: .*statements$//' | $STDBUF awk '{ print $3" "$1" "$0 }' | $STDBUF tr -s ' ' | $STDBUF cut --output-delimiter='	'  -d' ' -f'1,2,4,6-' | $STDBUF tr -s ' ' | $STDBUF tr -s '\t' | $STDBUF tr '\t' ' ' | $STDBUF sed 's/[0-9]s\ /\t/g' | $STDBUF tr ' ' '\t'
